...
export default {
  async fetch(request, env) {
    ...
    if (!env.OPENROUTER_API_KEY) {
      return json({ error: "Missing OPENROUTER_API_KEY secret." }, 500, origin);
    }
    ...
    try {
      const upstream = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://davetwchiu.github.io",
          "X-Title": "Humaniser"
        },
        body: JSON.stringify({
          model,
      ...
      try {
        ...
        return json({ error: "Could not parse OpenRouter response", details: raw }, 502, origin);
      }

      const content = parsedUpstream?.choices?.[0]?.message?.content;
      if (!content) {
        return json({ error: "No model output returned", details: parsedUpstream }, 502, origin);
      }
